...
typedef struct {
    ...
    SEL tx_setter;
    SEL getter;
} CGDynamicDelegateInfos;

@interface CGDynamicDelegate : NSProxy {
    const char *_key;
}


/**
 注意：如果 realDelegate 是被代理本身，例如：realDelegate 是 CGLimitedTextField，则有可能出现消息无限转发问题，应当尽量避免使用self.delegate = self;
 */
