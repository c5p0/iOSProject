...
#define CGLimitedTextFieldTypePriceRegExZero @"^[0][0-9]+$"
#define CGLimitedTextFieldTypePriceRegExContentFormat(limitedPrefix, limitedSuffix) [NSString stringWithFormat:@"^\\d{0,%ld}$|^(\\d{0,%ld}[.][0-9]{0,%ld})$", limitedPrefix, limitedPrefix, limitedSuffix]

#import "CGMatchManager.h"

@implementation CGMatchManager
...
+ (BOOL)matchLimitedTextTypePriceWithComponent:(id)component value:(NSString *)matchStr {
    ...
    // 1.匹配以0开头的数字
    NSPredicate *matchZero = [NSPredicate predicateWithFormat:TXRegExHeader, CGLimitedTextFieldTypePriceRegExZero];
    // 2.匹配两位小数、整数
    NSPredicate *matchValue = [NSPredicate predicateWithFormat:TXRegExHeader,CGLimitedTextFieldTypePriceRegExContentFormat((long)limitedPrefix, (long)limitedSuffix)];
