...
#import "CGDynamicDelegate.h"
#import <objc/message.h>

@interface CGDynamicDelegate ()

@property (assign, nonatomic) BOOL isWillBeForwardingLoop;

@end

@implementation CGDynamicDelegate
...
- (BOOL)respondsToSelector:(SEL)selector {
    ...
    /** 当 realDelegate 为 UITextField 时，能够响应 keyboardInputChangedSelection: 方法，此时返回 return YES，但实际上 CGDynamicDelegate 无法响应 keyboardInputChangedSelection:，因此会进入消息转发阶段！(注意：当 realDelegate.delegate = CGDynamicDelegate，则将进入消息无限转发) */
...
static void const * kCGDynamicDelegateInfosKey = &kCGDynamicDelegateInfosKey;

/**
 Just NSPointerFunctions sizeFunction.
 @return the size of CGDynamicDelegateInfos size
 */
static NSUInteger getDynamicDelegateInfosSizeFunction(const void *item) {
    return sizeof(CGDynamicDelegateInfos);
}
...
static Class tx_getDynamicDelegateClass(Class cls, NSString *suffix)
{
    ...
    return [CGDynamicDelegate class];
}
...
static inline CGDynamicDelegate *getDynamicDelegate(NSObject *delegatingObject, const CGDynamicDelegateInfos *info, const char *key) {
    
    CGDynamicDelegate *dynamicDelegate = [delegatingObject getDynamicDelegateWithCls:tx_getDynamicDelegateClass(delegatingObject.class, @"Delegate") key:key];
    
    id (*getterFunc)(id, SEL) = (id (*)(id, SEL)) objc_msgSend;
    id getterDelegate = getterFunc(delegatingObject, info->getter);
    
    if ([getterDelegate isKindOfClass:[CGDynamicDelegate class]]) { return dynamicDelegate; }
...
- (CGDynamicDelegate *)getDynamicDelegateWithCls:(Class)cls key:(const char *)key {
    
    __block CGDynamicDelegate *dynamicDelegate;
...
+ (void)tx_registerDynamicDelegateWithName:(NSString *)delegateName {
    ...
    CGDynamicDelegateInfos *dynamicDelegateInfos = (__bridge void *)([delegateInfos objectForKey:(__bridge id _Nullable)kCGDynamicDelegateInfosKey]);
    ...
    CGDynamicDelegateInfos info = {
        setter, tx_setter, getter
    };
    
    [delegateInfos setObject:(__bridge id)&info forKey:(__bridge id _Nullable)kCGDynamicDelegateInfosKey];
    
    dynamicDelegateInfos = (__bridge void *)[delegateInfos objectForKey:(__bridge id _Nullable)kCGDynamicDelegateInfosKey];
    
    IMP setterImp = imp_implementationWithBlock(^(NSObject *delegatingObject, id delegate) {
        
        CGDynamicDelegate *dynamicDelegate = getDynamicDelegate(delegatingObject, dynamicDelegateInfos, kTXOperationDelegateKey);
