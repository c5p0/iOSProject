...
@interface DQDownLoader () <NSURLSessionDataDelegate>
{
    // 记录文件临时下载大小
    long long _tmpSize;
    // 记录文件总大小
...
- (void)downLoader:(NSURL *)url {
    ...
    if ([url isEqual:self.dataTask.originalRequest.URL]) {
        ...
        // 下载失败
        // 判断当前的状态, 如果是暂停状态
        if (self.state == DQDownLoadStatePause) {
            // 继续
    ...
    //     return
    if ([DQFileTool fileExists:self.downLoadedPath]) {
        // UNDO: 告诉外界, 已经下载完成;
        //        NSLog(@"已经下载完成");
        self.state = DQDownLoadStatePauseSuccess;
        return;
    }
    ...
    // 2.2 不存在: 从0字节开始请求资源
    //     return
    if (![DQFileTool fileExists:self.downLoadingPath]) {
        // 从0字节开始请求资源
        [self downLoadWithURL:url offset:0];
        return;
    }
    
    _tmpSize = [DQFileTool fileSize:self.downLoadingPath];
...
- (void)pauseCurrentTask {
    if (self.state == DQDownLoadStateDownLoading) {
        self.state = DQDownLoadStatePause;
...
- (void)resumeCurrentTask {
    if (self.dataTask && self.state == DQDownLoadStatePause) {
        [self.dataTask resume];
        self.state = DQDownLoadStateDownLoading;
    }
...
- (void)cacelAndClean {
    [self cacelCurrentTask];
    [DQFileTool removeFile:self.downLoadingPath];
...
-(void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    if (_tmpSize == _totalSize) {
        // 1. 移动到下载完成文件夹
        //        NSLog(@"移动文件到下载完成");
        [DQFileTool moveFile:self.downLoadingPath toPath:self.downLoadedPath];
        // 2. 取消本次请求
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error == nil) {
        ...
        if (-999 == error.code) {
            ...
            self.state = DQDownLoadStatePauseFailed;
        }
...
#pragma mark - 事件/数据传递
- (void)setState:(DQDownLoadState)state {
    // 数据过滤
    if(_state == state) {
        return;
    }
    ...
    if (_state == DQDownLoadStatePauseSuccess && self.successBlock) {
        self.successBlock(self.downLoadedPath);
    }
    
    if (_state == DQDownLoadStatePauseFailed && self.faildBlock) {
        self.faildBlock();
    }
