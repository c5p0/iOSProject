...
#import "NSString+Extension.h"
@interface DQDownLoadManager()<NSCopying, NSMutableCopying>

//@property (nonatomic, strong) DQDownLoader *downLoader;
...
static DQDownLoadManager *_shareInstance;
...
// key: md5(url)  value: DQDownLoader
...
- (void)downLoader:(NSURL *)url downLoadInfo:(DownLoadInfoType)downLoadInfo progress:(ProgressBlockType)progressBlock success:(SuccessBlockType)successBlock failed:(FailedBlockType)failedBlock {
    ...
    DQDownLoader *downLoader = self.downLoadInfo[urlMD5];
    if (downLoader == nil) {
        downLoader = [[DQDownLoader alloc] init];
