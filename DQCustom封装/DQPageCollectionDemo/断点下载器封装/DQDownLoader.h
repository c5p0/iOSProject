...
typedef NS_ENUM(NSUInteger, DQDownLoadState) {
    DQDownLoadStatePause,
    DQDownLoadStateDownLoading,
    DQDownLoadStatePauseSuccess,
    DQDownLoadStatePauseFailed
};
...
typedef void(^StateChangeType)(DQDownLoadState state);


// 一个下载器, 对应一个下载任务
// DQDownLoader -> url
@interface DQDownLoader : NSObject
...
/// 数据
/// 事件&数据
@property (nonatomic, assign, readonly) DQDownLoadState state;
